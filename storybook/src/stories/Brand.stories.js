...
import { bestOn, resolveFamily } from '../theme.js';
...
export const Overview = (args, context) => {
  ...
  <section class="band"><div class="wrap grid" style="grid-template-columns:repeat(auto-fit,minmax(280px,1fr));align-items:start">
    <div class="stack">
      ...
      <p class="caption">Headline · <strong>${esc(f.heading)}</strong>${f.heading && resolveFamily(f.heading) !== f.heading ? ` <span class="mono">(shown as ${esc(resolveFamily(f.heading))})</span>` : ''}</p>
    </div>
  ...
  <section class="band"><div class="wrap stack-lg">
    ...
    <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(260px,1fr));align-items:start">
      ...
      <div class="stack">
        ...
        <p class="caption">Switch brand in the toolbar → the whole page re-themes with its real fonts.</p>
      </div>
    </div>
  </div></section>`;
};
